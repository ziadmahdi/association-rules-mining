...
class FPTree
{
     ...
     */
    private static function buildHeaderTable($frequent)
    {
        $headers = [];
     ...
     */
    private function buildFPTree(&$transactions, &$root_value, &$root_count, &$frequent, &$headers)
    {
        ...
        foreach ($transactions as $transaction) {
            ...
            usort($sorted_items, function ($a, $b) use ($frequent) {
                return $frequent[$a] == $frequent[$b] ? 0 : ($frequent[$a] > $frequent[$b] ? -1 : 1
                );
            });
     ...
     */
    private function insertTree(&$items, &$node, &$headers)
    {
        $first = $items[0];
     ...
     */
    private function mineSubTrees($threshold)
    {
        $patterns = [];
